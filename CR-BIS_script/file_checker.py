...
def process_next_tilt_file(file_path):
    ...
    try:
        ...
        if numbers:
            ...
            print(f"NEXT tilt angle: {next_tilt_angle}, Stage: {next_Stage_X},{next_Stage_Y},{next_Stage_Z}")
    ...
    try:
        ...
        print(f"delete failed: {e.errno}, error: {e.strerror}")
...
def tilt_stage_to(next_tilt_angle,next_Stage_X,next_Stage_Y,next_Stage_Z):
    ...
    print(f"{current_A},{math.radians(current_A)}")
...
def main():
    ...
    print(f"start checking: {directory}")
    while True:
        # check file
